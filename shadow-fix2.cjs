...
// 1. 修复被破坏的 .shadow 规则（$1 残骸 + 已有的 transition 行），并带上 lift 规则
...
// 7. types 声明合并
{
  ...
  if (!s.includes('pulseWindowShadow(): Promise<void>')) {
    insertAfterLine('src/types.ts',
      'export interface LauncherApi {',
      '  /** 模式翻转时通知主进程：阴影窗播放「抬升-回落」脉冲。 */\n  pulseWindowShadow(): Promise<void>',
      'types LauncherApi')
  } else { n++; console.log('ok: types LauncherApi (already)') }
}

// 8. demo-api 桩
{
  let s = fs.readFileSync('src/demo-api.ts', 'utf8')
  if (!s.includes('pulseWindowShadow')) {
    insertAfterLine('src/demo-api.ts',
      '  presetsBuiltin: async () => [],',
      '  pulseWindowShadow: async () => undefined,',
      'demo-api stub')
  } else { n++; console.log('ok: demo-api stub (already)') }
}

// 9. ipc handler + import
{
  let s = fs.readFileSync('electron/ipc.ts', 'utf8')
  if (!s.includes('IPC.windowFlipPulse')) {
