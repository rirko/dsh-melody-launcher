...
import type { PackPluginEntry, ProfileRepositoryImportMode, ProfileRepositoryImportPreview } from '../../types'
...
export function ProfileRepositoryImportDialog({ open, url, preview, busy, error, onUrlChange, onAnalyze, onConfirm, onClose }: ProfileRepositoryImportDialogProps) {
  ...
  if (!open) return null
  const unresolved = preview?.plugins.some(plugin => (plugin.match === 'missing') || (plugin.match === 'ambiguous' && !resolutions[plugin.packageName])) ?? true
  const canConfirm = Boolean(preview) && !busy && name.trim().length > 0
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target && !busy) onClose() }}>
      <section className="modal pack-install-dialog profile-repository-dialog" role="dialog" aria-modal="true" aria-labelledby="profile-repository-title">
        <header>
          <div><GitFork size={19} /><h2 id="profile-repository-title">从 GitHub 导入 Profile</h2></div>
        ...
        <footer>
          ...
          <button type="button" className="primary-command" disabled={!canConfirm} onClick={() => onConfirm(mode, name.trim(), overwrite, resolutions)}><Download size={16} />确认导入</button>
        </footer>
      </section>
    </div>
  )
